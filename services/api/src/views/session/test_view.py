...
def test_session_username(api_conn, test_conn):
    ...
    sut = initialize_view(conn=api_conn, secret_key=secret_key)

    username, result = sut.session_username(token)
...
def test_session_username_with_invalid_hmac_tag(api_conn, test_conn):
    ...
    username, result = sut.session_username(malformed_token)
...
def test_session_username_with_bad_base64_tag(api_conn, test_conn, bad_token):
    ...
    username, result = sut.session_username(bad_token)
